...
export default class TokenRefreshService {
    ...
    return axios
      ...
      .then(response => {
        ...
        const { code, result, message } =
          formatResponse && isFunction(formatResponse)
   ...
   * @returns
   */
  public async refreshTokenCheck(config: object, options: object) {
    // 初始化 config
    this.config = config;
    // 初始化 options
        ...
        return this.requests.push((token: string) => {
          return new Promise(resolve => {
            if (token && (this.config as Recordable)?.requestOptions?.withToken !== false) {
              // jwt token
              (this.config as Recordable).headers[authorizationKey] = this.options
