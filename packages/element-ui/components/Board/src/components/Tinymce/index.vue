...
import { urlMap } from '@/settings/constants.js'
export default {
  components: {
    Editor
  },
  ...
  data() {
    return {
      ...
      fullscreen: false,
      content: this.value,
      host: urlMap[process.env.VUE_APP_BASE_API]
    }
  },
  computed: {
    // 初始化配置
    ...
    addPlaceHoldersPlugin() {
      tinymce.PluginManager.add('placeholder', function(editor) {
        ...
        const Label = function() {
          ...
          const placeholder_attrs = editor.settings.placeholder_attrs || { style: { position: 'absolute', top: 0, left: 0, color: '#888', padding: '15px', width: '100%', overflow: 'hidden', 'white-space': 'pre-wrap' }}
