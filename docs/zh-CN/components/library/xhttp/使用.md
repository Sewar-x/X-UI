...
| expires（选填）         | number | token  过期时间                                              |
| tokenExpiresKey（必填） | string | token 过期时间本地存储的 key，如果没有传入expires，则自动从本地通过 tokenExpiresKey 获取 token 过期时间 |
| refreshIdKey（必填）    | string | token 刷新id 本地存储的 key                                  |
| refreshId（选填）       | string | token 刷新id                                                 |
...
export default createXhttp({
    ...
    refreshTokenConfig: { // token 自动刷新配置
        ...
        refreshIdKey: 'test-refreshIdKey', // 存储 refreshId 刷新 token id 的 key
